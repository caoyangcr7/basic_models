...
# reorg layer ----> extract image patch from original image
def reorg(x, stride):
    return tf.extract_image_patches(x, [1, stride, stride, 1],
                        [1, stride, stride, 1], [1,1,1,1], padding="VALID")

# for coco dataset , there  is 80 classes totally,so last_channels is 425
def darknet_19(images, n_last_channels=425):
    """ darknet for yolo 2"""
    ...
    net = conv2d(net, 512, 3, 1, name="conv5_5")  # the 16th or 17 th layer
    # -------
    shortcut = net
    # -------
    ...
    # ---------the darknet that drops the last conv layer and fc layer
    # shortcut
